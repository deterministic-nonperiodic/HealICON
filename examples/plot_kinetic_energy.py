...
This script demonstrates how to compute the kinetic energy spectrum of the 
atmosphere using two different but mathematically equivalent approaches in 
HealICON:
1. Directly from the U and V wind components using a spin-1 vector spherical 
   harmonic transform.
2. From Divergence and Vorticity using standard scalar spherical harmonic 
   transforms.
...
def main():
    ...
    spc_uv = compute_spectrum(ds_hp, var_name=['u', 'v'], lmax=lmax,
                              spectrum_type='kinetic').compute()
    ...
    spc_dv = compute_spectrum(ds_dv, var_name=['divergence', 'vorticity'], lmax=lmax,
                              spectrum_type='kinetic').compute()
