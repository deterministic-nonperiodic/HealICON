...
def plot_tides_comparison():
    saber_file = '../saber_ut_tides_zm.nc'
    icon_file = './output_tides_zm.nc'
    saber_raw = '../saber_healpix.nc'
    
    if not os.path.exists(saber_file) or not os.path.exists(icon_file):
        print("Required files missing.")
        return

    ds_saber = xr.open_dataset(saber_file)
    ds_icon = xr.open_dataset(icon_file).sortby(['lat', 'height'])
    
    # Extract mean altitude in km across all valid cells and UT bins
    ...
    out_path = 'tides_ut_comparison.png'
