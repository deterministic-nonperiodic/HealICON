...
def compute_spectrum(ds: xr.Dataset, var_name: str, lmax: int = None) -> xr.Dataset:
    ...
    sep = "\n" if history else ""
    out_ds.attrs['history'] = f"{history}{sep}Computed angular power spectrum (Cl) up to lmax={lmax} for {var_name}."
    return out_ds
...
def compute_vorticity_divergence(ds: xr.Dataset, u_var: str, v_var: str, lmax: int = None) -> xr.Dataset:
    """
    ...
    out_ds['divergence'].attrs = {'standard_name': 'divergence_of_wind', 'units': 's-1', 'grid_mapping': 'healpix'}
    
    out_ds['vorticity'] = vor.assign_coords(cell=ds.cell)
    out_ds['vorticity'].attrs = {'standard_name': 'atmosphere_relative_vorticity', 'units': 's-1', 'grid_mapping': 'healpix'}
