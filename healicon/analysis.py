...
def _helmholtz_block(u_block, v_block, lmax, nside, is_nested):
    ...
    for i in range(n):
        ...
        psi[i] = ensure_original_order(hp.alm2map(psi_alm, nside, lmax=lmax) * _EARTH_RADIUS_M,
                                       'nested' if is_nested else 'ring')

        # Velocity potential χ: D = ∇²χ  →  χ_lm = almE_lm / fl  (× a for m²/s)
        chi_alm = np.where(l_arr > 0, almE / fl_safe, 0.0 + 0.0j)
        chi[i] = ensure_original_order(hp.alm2map(chi_alm, nside, lmax=lmax) * _EARTH_RADIUS_M,
                                       'nested' if is_nested else 'ring')
...
def _vorticity_divergence_block(u_block, v_block, lmax, nside, is_nested):
    ...
    for i in range(u_2d.shape[0]):
        ...
        div_out[i] = ensure_original_order(hp.alm2map(div_alm, nside, lmax=lmax),
                                           'nested' if is_nested else 'ring')
        vor_out[i] = ensure_original_order(hp.alm2map(vor_alm, nside, lmax=lmax),
                                           'nested' if is_nested else 'ring')
