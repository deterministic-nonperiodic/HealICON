...
def process_file(
    ...
    if user_mapping:
        ...
        if cf_map:
            ...
            logger.warning("No variable mapping provided and no CF conventions found. Processing all data variables.")
    ...
    if nside is None:
        if not spatial_dims:
            raise ValueError("nside must be provided if spatial dimensions cannot be determined from the dataset")
        n_orig = 1
