...
def interpolate_unstructured(ds: xr.Dataset, nside: int, source_lon_name: str, source_lat_name: str,
                             spatial_dim: str, use_gpu: bool = False) -> xr.Dataset:
    """
    ...
    for var_name, da in ds.data_vars.items():
        ...
        # Assign coordinates
        interpolated_da = interpolated_da.assign_coords(cell=out_ds.cell, lon=out_ds.lon,
                                                        lat=out_ds.lat)
...
def interpolate_to_healpix(ds: xr.Dataset, nside: int = None, use_gpu: bool = False) -> xr.Dataset:
    ...
    if 'rad' in lon_units or 'rad' in lat_units:
        ...
        lon_max = float(
            abs(lon_data).max().compute() if hasattr(lon_data, 'compute') else abs(lon_data).max())
        lat_max = float(
            abs(lat_data).max().compute() if hasattr(lat_data, 'compute') else abs(lat_data).max())
    ...
    if is_radians:
        logger.info(
            f"Coordinates {lon_name}, {lat_name} appear to be in radians. Converting to degrees.")
    ...
    if is_regular_grid:
        logger.info(
            f"Detected regular latitude-longitude grid with dimensions ({lat_name}, {lon_name}).")
        ...
        out_ds = interpolate_unstructured(ds, nside, lon_name, lat_name, spatial_dim,
                                          use_gpu=use_gpu)

    elif len(lon_dims) == 2 and lon_dims == lat_dims:
        # Curvilinear grid (flatten it and treat as unstructured)
        logger.info(
            f"Detected 2D curvilinear grid with dimensions {lon_dims}. Flattening to unstructured.")
        spatial_dim = f"{lon_dims[0]}_{lon_dims[1]}"
        ds_flat = ds.stack({spatial_dim: lon_dims})
        out_ds = interpolate_unstructured(ds_flat, nside, lon_name, lat_name, spatial_dim,
                                          use_gpu=use_gpu)
